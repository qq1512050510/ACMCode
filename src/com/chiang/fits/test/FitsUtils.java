...
public class FitsUtils {
	...
	public static void main(String[] argv) {
		...
		for (int na = 0; na < argv.length; na++) {
			// 校验文件
			FitsUtils.validateFile(argv[na]);

			System.out.println("-- Test finished -----------------");
			System.out.printf("执行时间：%s分钟", (System.currentTimeMillis() - currentTime) / 1000 / 60);
		}
	...
	private static void fileValidate(FitsFile file, Integer i) {
		...
		System.out.println("  " + i + ": >" + hdr.getName() + "< of type >" + Fits.getType(type) + "< with " + noKw
				+ " keywords" + " and " + size + " bytes of data");
		System.out.println("   Keywords:");
		ListIterator itr = hdr.getKeywords();
		System.out.printf("iter i:%s satrat", i);
		int whileCount = 0;
		while (itr.hasNext()) {
			FitsKeyword kw = (FitsKeyword) itr.next();
			System.out.printf("while Count ：%s", whileCount++);
		...
		// 不同类型 对应不同的处理方式
		dealWhithFileByTye(type, hdu);
	}

	private static void dealWhithFileByTye(int type, FitsHDUnit hdu) {
		if (type == Fits.IMAGE) {
			...
			for (int n = 0; n < naxis.length; n++)
				System.out.println(
						"   Axis " + n + ": " + naxis[n] + ",  " + crpix[n] + ",  " + crval[n] + ",  " + cdelt[n]);
			System.out.println("\n");
	...
	public static void validateFile(String fileDir) {
		FitsFile file = null;
		file = openFits(fileDir);
		// if(file)
		int noHDU = file.getNoHDUnits();
		System.out.println("FITS file has " + noHDU + " HDUnits");

		for (int i = 0; i < noHDU; i++) {
			System.out.println("File Validate Start!");
			fileValidate(file, i);
			System.out.println("File Validate End!");
		}
	}
}